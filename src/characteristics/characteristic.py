...
one particular *characteristic* -- one toleranced position, flatness, or
...
tolerance types are shaped, and *oriented*, differently:
  ...
  - A **flatness** tolerance is a pair of parallel planes, oriented to
    the *measured surface's own* best orientation: it cares about
    deviation along one direction only (the surface's normal) and not at
    all about deviation within the planes (sideways).
  - A **parallelism** tolerance is also a pair of parallel planes, one
    ...
    but slightly tilted relative to its datum has a flatness normal and a
...
Tolerance = Union[PositionTolerance, FlatnessTolerance, ParallelismTolerance]
...
class Characteristic:
    ...
    target_indices : which target point(s) this characteristic involves,
        ...
        is given. A single index for most position/parallelism
        characteristics; several for a flatness callout spanning multiple
        measured points on one surface (see the module docstring's note
        on what step 4 does and doesn't do with several points).
    tolerance : the tolerance type and its zone size -- a
        `PositionTolerance`, `FlatnessTolerance`, or `ParallelismTolerance`.
    datum : the external reference `ParallelismTolerance` measures
        against; ignored by `PositionTolerance` and `FlatnessTolerance`
...
def evaluate_characteristic(
    ...
    flatness callout's classic max-minus-min across several correlated
    points is a harder statistical question -- order statistics of
    correlated Gaussians -- than a direct projection, and isn't needed to
    show that different tolerance types on the *same* point project
    differently, which is this step's gate). A multi-point aggregate is a
    natural next elaboration, not built here (CLAUDE.md §9).
    """
